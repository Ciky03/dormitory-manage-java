...
import cloud.ciky.sms.config.AliyunSmsProperties;
import cloud.ciky.sms.service.SmsService;
...
public class AuthService {
    ...
    public void logout() {
        ...
        if (expireTimeOpt.isEmpty()) {
            // token 永不过期则永久加入黑名单
            redisTemplate.opsForValue().set(RedisConstants.Auth.BLACKLIST_TOKEN + jti, "", 3, TimeUnit.DAYS);
        }
    }
}
