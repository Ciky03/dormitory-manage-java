package cloud.ciky.auth.oauth2.extension.captcha;
 ...
 * <p>
 * 验证码模式身份验证提供者
 * 处理基于用户名和密码、图形验证码的身份验证
 * </p>
 *
 * @author ciky
 * @since 2025/12/12 12:02
 */
@Slf4j
public class CaptchaAuthenticationProvider implements AuthenticationProvider {
    ...
    public CaptchaAuthenticationProvider(AuthenticationManager authenticationManager,
        ...
        Assert.notNull(codeGenerator, "codeGenerator cannot be null");
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // 验证码校验
        String captchaKey = String.valueOf(additionalParameters.get(CaptchaParameterNames.CAPTCHA_KEY));
        String captchaCode = String.valueOf(additionalParameters.get(CaptchaParameterNames.CAPTCHA_CODE));

        String cacheCaptchaCode = redisTemplate.opsForValue().get(RedisConstants.Captcha.IMAGE_CODE + captchaKey);

        if (CharSequenceUtil.isBlank(cacheCaptchaCode)) {
        ...
        String username = String.valueOf(additionalParameters.get(OAuth2ParameterNames.USERNAME));
        String password = String.valueOf(additionalParameters.get(OAuth2ParameterNames.PASSWORD));
